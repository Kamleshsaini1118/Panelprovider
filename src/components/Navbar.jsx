    ...
    <header className="w-full fixed top-0 left-0 shadow-md z-50 bg-[#0a1128]">
      <div className="container mx-auto flex items-center justify-between px-6 py-4 md:px-20">
        ...
        <button className="hidden md:block relative p-[1px] rounded-full bg-gradient-to-r from-green-300 via-orange-300 to-pink-300">
          <a className="flex items-center bg-[#080e26] text-white text-base px-6 py-2 rounded-full shadow-lg font-medium hover:bg-white hover:text-black transition w-full h-full"
      ...
      {/* Mobile Menu - Right Side Slide-in */}
      <div className={`fixed top-0 right-0 h-full w-64 bg-[#0a1128] shadow-lg transform ${menuOpen ? "translate-x-0" : "translate-x-full"} transition-transform duration-300 ease-in-out md:hidden`}>
        ...
        <nav className="flex flex-col items-start p-6 space-y-4 text-white mt-16">
          ...
          <div className="w-full">
            <button className="w-full text-left hover:text-[#FFB90C] flex justify-between" onClick={() => setDropdownOpen(!dropdownOpen)}>
              Our Services <span>{dropdownOpen ? "▲" : "▼"}</span>
            </button>
            <ul className={`overflow-hidden transition-all duration-300 ease-in-out ${dropdownOpen ? "max-h-40 opacity-100" : "max-h-0 opacity-0"}`}>
              <li><a href="/sharing-sites-list" className="block px-4 py-2 hover:bg-gray-200">Sharing Sites List</a></li>
              <li><a href="/rental-sites-list" className="block px-4 py-2 hover:bg-gray-200">Rental Sites List</a></li>
              <li><a href="/self-deposit-and-withdrawal" className="block px-4 py-2 hover:bg-gray-200">Self Deposit & Withdrawal</a></li>
            </ul>
